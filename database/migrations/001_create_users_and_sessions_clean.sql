-- Migration: Create users, sessions, and nda_signatures tables (CLEAN VERSION)
-- Date: 2025-12-20
...
ALTER TABLE users ADD CONSTRAINT chk_user_type 
    CHECK (user_type IN ('waitlist', 'investor', 'admin'));

ALTER TABLE users ADD CONSTRAINT chk_status 
    CHECK (status IN ('active', 'suspended', 'deleted'));

ALTER TABLE users ADD CONSTRAINT chk_investor_status_values 
    CHECK (investor_status IN ('pending_nda', 'nda_signed', 'active', 'rejected') OR investor_status IS NULL);

ALTER TABLE users ADD CONSTRAINT chk_user_type_investor_status 
    CHECK (
...
ALTER TABLE users ADD CONSTRAINT fk_admin_approved_by 
