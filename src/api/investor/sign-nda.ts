...
export async function signNDA(c: Context<{ Bindings: Bindings }>) {
  try {
    ...
    const adminEmail = c.env?.ADMIN_EMAIL || 'admin@risivo.com';
    const fromEmail = c.env?.FROM_EMAIL || 'hello@risivo.com';

    console.log('[NDA] Email config - SendGrid key exists:', !!sendgridKey);
    console.log('[NDA] Email config - Admin email:', adminEmail);
    console.log('[NDA] Email config - From email:', fromEmail);
    ...
    if (sendgridKey) {
      ...
      try {
        console.log('[NDA] Sending investor confirmation email to:', user.email);
        await emailService.sendInvestorNDAConfirmation({
          email: user.email,
          firstName: user.first_name
        });
        console.log('[NDA] ✅ Investor confirmation email sent successfully to:', user.email);
      } catch (emailError) {
