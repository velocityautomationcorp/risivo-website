...
cmsAdmin.get("/pages", async (c) => {
  try {
    const supabaseCMS = getSupabaseCMSAdmin(
      c.env?.SUPABASE_URL,
...
cmsAdmin.post("/pages", async (c) => {
  try {
    ...
    console.log(
      "🔑 Supabase URL:",
      c.env?.SUPABASE_URL ? "configured" : "missing"
    );
...
cmsAdmin.delete("/pages/:id", async (c) => {
  ...
  try {
    ...
    const { error } = await supabaseCMS.from("cms_pages").delete().eq("id", id);
...
cmsAdmin.delete("/media/:id", async (c) => {
  ...
  try {
    ...
    if (media) {
      ...
      await supabaseCMS.storage.from("media").remove([media.storage_path]);
    }

    // Delete from database
    const { error } = await supabaseCMS.from("cms_media").delete().eq("id", id);
...
cmsAdmin.get("/translations", async (c) => {
  ...
  try {
    ...
    let query = supabaseCMS.from("cms_translations").select("*").order("key");
